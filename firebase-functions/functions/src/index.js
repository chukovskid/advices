...
exports.testFunction = exports.helloWorld = exports.informUserForUpenCall = exports.sendToDevice = exports.sendToTopic = exports.analyzeContract = exports.notificationForNewMessage = exports.notifyNewCalls = exports.callUser = exports.getCustomToken = void 0;
...
exports.callUser = functions.https.onCall(async (data, context) => {
    ...
    const tokens = querySnapshot.docs.map((snap) => snap.id); // TODO check the  token!
...
exports.notifyNewCalls = functions.firestore
...
exports.notificationForNewMessage = functions.firestore.document('/conversation/groups/chats/{chatId}').onWrite(async (snapshot) => {
    ...
    let receiverId = members.filter((member) => member !== senderId)[0];
...
async function extractKeyClausesAndTerms(preprocessedText) {
...
async function identifyIssuesAndRisks(preprocessedText) {
...
async function generateExplanationsAndRecommendations(issuesAndRisks) {
...
async function compareWithTemplates(preprocessedText) {
...
function preprocess(contractText) {
...
exports.sendToTopic = functions.firestore.document('puppies/{puppyId}').onCreate(async (snapshot) => {
...
//Test function - remove it
exports.sendToDevice = functions.firestore.document('orders/{orderId}').onCreate(async (snapshot) => {
...
//Test function - remove it
exports.informUserForUpenCall = functions.firestore.document('orders/{orderId}').onCreate(async (snapshot) => {
    const order = snapshot.data();
    const querySnapshot = await db
        .collection('users')
        .doc(order.seller)
        .collection('tokens')
        .get();
    const tokens = querySnapshot.docs.map((snap) => snap.id); // it seems like we give the receaverId here
    const payload = {
        notification: {
            title: 'New Order!',
            body: `you sold a ${order.product} for ${order.total}`,
...
exports.helloWorld = functions.https.onRequest(async (request, response) => {
    ...
    await (0, email_1.sendEmail)("cukovskidimitar@gmail.com", client.email, "body 123", "HTML Client 123");
    const lawyerRef = await db
        .collection('users')
        .doc("Lk37HV68oaPxOA8AHpNqcSoFgEA3").get();
    const lawyer = lawyerRef.data();
    await (0, email_1.sendEmail)("cukovskidimitar@gmail.com", lawyer.email, "body 123", "HTML Lawyer 123");
...
exports.testFunction = functions.https.onCall(async (data) => {
    ...
    try {
        await (0, email_1.sendEmail)('cukovskidimitar@gmail.com', "test proba 1", "This is the body", "this is the HTML");
    }
    catch (error) {
        return error;
    }
